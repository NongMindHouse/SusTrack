...
const PopupIndicator: React.FC<Props> = ({ activeTab, isOpen, onTabClick, onClose, onSubmit }) => {
      ...
      <div>
        {targets.map((target) => (
          <div>
            ...
            {goalObject
              .filter((item) => item.TargetsNumber.toString() === target)
              .map((ind) => (
                <div
                  key={ind.IndicatorsNumber}
                  className="bg-gray-200 my-3 rounded-md mx-4 p-2
  ...
  return (
    ...
    >
      <div className="bg-[rgba(0,0,0,0.6)] w-full h-full flex justify-center items-center" onClick={handleOverlayClick}>
        <div
          className="md:w-[500px] lg:w-[800px] xl:w-[1000px] 2xl:w-[1400px]
      p-3 rounded-lg flex flex-col relative"
        >
          ...
          <div className="bg-white p-5 rounded-b-lg h-[540px] 2xl:h-[700px]">
            {/* BANNER GOAL */}
            <div
              className="absolute top-[75px] -left-[40px] w-[80px] h-[120px]
          flex flex-col justify-center items-center text-white"
            ...
            >
              ...
              />
              <p className="text-[14px]">เป้าหมาย</p>
              <p className="text-[20px] font-extrabold">{activeTab + 1}</p>
            </div>
            {/* CONTAINTER */}
            <form>
                ...
                <div className="">
                  ...
                  className="w-7 h-7 mr-4 accent-green-600" /> */}
                    <p className="inline-block">
                      {" "}
                      เป้าหมายที่ {activeTab + 1}{" "}
                      {indicatorData.find((item) => item.SDGNumber === activeTab + 1)?.SDGTitle}
                    </p>
                  </p>
                </div>
                {/* INDICATOR CONTAINER */}
                <div className="bg-gray-100 p-3 rounded-lg overflow-auto">
                  <div>{loadIndicator(activeTab)}</div>
                </div>
                {/* SUBMIT */}
